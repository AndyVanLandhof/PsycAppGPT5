...
export default function ScenarioRunner({ topicId, topicTitle, onClose }) {
  ...
  useEffect(() => {
    ...
    setItems(getScenarioSet(topicId, 2));
  }, [topicId]);
  ...
  const submit = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      const it = items[i];
      const payload = { prompt: it.prompt, markscheme: it.markscheme || [], studentAnswer: answers[i] || '', maxMarks: it.max };
      try {
        const r = await markScenario(topicTitle || topicId, payload, callAIWithPublicSources);
        const m = Math.max(0, Math.min(it.max, Number(r.raw)||0));
        raw += m; max += it.max; perItem.push(m); snippets.push(r.rationale || '');
      } catch(_){ const m = Math.min(it.max, Math.round(((answers[i]||'').split(/\s+/).length/35) * it.max)); raw+=m; max+=it.max; perItem.push(m); snippets.push(''); }
    }
  ...
  if (result) return (
    <div className="space-y-3">
      <div className="font-semibold">Scenario/Application Results</div>
      <div className="text-xs text-gray-600">Source: <span className="px-2 py-0.5 rounded bg-gray-100">{isAiCached('scenario', topicId) ? 'AI' : 'Mock'}</span></div>
      ...
      <div className="flex gap-2">
        <button className="px-3 py-2 rounded bg-indigo-600 text-white" onClick={() => { sessionStorage.removeItem(key); setItems(getScenarioSet(topicId,2)); setAnswers({}); setResult(null); }}>Try Again</button>
    ...
    <div className="space-y-4">
      {items.map((it, i) => (
        <div key={i} className="space-y-2">
          ...
          <textarea rows="4" className="w-full border rounded p-2" value={answers[i]||''} onChange={(e)=> setAnswers({ ...answers, [i]: e.target.value })} />
        </div>
      ))}
